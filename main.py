...
# CRUD: Read(lectura) GET ALL: Leeremos todos los cursos que haya en la base de datos
...
def crear_curso(curso:Curso):
    curso.id = str(uuid.uuid4()) # Usamos UUID para generar un ID único e irrepeteble
    cursos_db.append(curso)
    return curso

# CRUD: Read (lectura) GET (ividual): Leeremos el curso que coincida con el ID que indiquemos
@app.get("/cursos/{curso_id}", response_model=Curso)
def obtener_curso(curso_id:str):
    curso = next((curso for curso in cursos_db if curso.id == curso_id), None) # Con Next tomamos la primera coincidencía del array devuelto
    if curso is None:
...
def actualizar_curso(curso_id:str, curso_actualizado:Curso):
    ...
    index = cursos_db.index(curso) # Buscamos el índice exacto donde está el curso en nuestra lista (BD)
...
@app.delete("/cursos/{curso_id}", response_class=Curso)
